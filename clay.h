...
#define CLAY(...) \
	for (\
		CLAY__ELEMENT_DEFINITION_LATCH = (Clay__OpenElement(CLAY__CONFIG_WRAPPER(Clay_ElementDeclaration, __VA_ARGS__)), 0); \
...
#define CLAY_REUSE(...) \
	for (\
		CLAY__ELEMENT_DEFINITION_LATCH = (Clay__OpenElementPointer(__VA_ARGS__), Clay__ElementPostConfiguration(), 0); \
		CLAY__ELEMENT_DEFINITION_LATCH < 1; \
		++CLAY__ELEMENT_DEFINITION_LATCH, Clay__CloseElement() \
	)
...
void Clay__OpenElementPointer(const Clay_ElementDeclaration* declaration) {
    Clay_Context* context = Clay_GetCurrentContext();
...
void Clay__OpenElement(const Clay_ElementDeclaration declaration) {
    Clay__OpenElementPointer(&declaration);
}
